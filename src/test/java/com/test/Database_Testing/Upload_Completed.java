...
public class Upload_Completed {
	 ...
	 @Test
	    ...
	    private void executeAndPrintQuery(Connection connection) throws SQLException {
	        ...
	        String query1 = "SELECT id,name,datalocation,arrival_date,totalImages FROM `slidebatch` WHERE `process_status` = 2 AND `arrival_date` < DATE_SUB(NOW(), INTERVAL 1 DAY);";
